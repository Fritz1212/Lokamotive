    ...
    try {
      ...
      // GET RECOMMENDED ROUTES
        ...
        db.query(query, [id], (err, results) => {
          ...
          ws.send(JSON.stringify({ status: "success", data: user }));
        });
      }
...
const mockMessage = JSON.stringify({
  ...
  userId: "user123",
  start: "Rumah Talenta BCA",
  destination: "Central park mall",
  startLat: -6.588498349789999,
  startLng: 106.88241288219487,
  destinationLat: -6.176945295983249,
  destinationLng: 106.7909251515093,
  preference: "fastest"
});

// Simulate WebSocket object with a mock `send` function
const mockWebSocket = {
  send: (response) => {
    console.log("Mock WebSocket Response:", response);
  }
};

// Manually trigger the routeController function
(async () => {
  await routeController.getRecommendedRoutesWS(mockMessage, mockWebSocket);
})();

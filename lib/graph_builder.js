...
class GraphBuilder {
  async buildGraph() {
    ...
    for (let i = 0; i < mrtStationNames.length - 1; i++) {
      ...
      graph.addEdge(origin, destination, "MRT", 3, { line: "MRT Jakarta" });
      graph.addEdge(destination, origin, "MRT", 3, { line: "MRT Jakarta" });
    }

    // 🚝 **3. LRT Integration**
    const lrtStations = await LRTService.getAllStations();

    console.log(lrtStations);
    ...
    for (let i = 0; i < lrtStations.length - 1; i++) {
      ...
      graph.addEdge(origin, destination, "LRT", 4, { line: "LRT Jakarta" });
      graph.addEdge(destination, origin, "LRT", 4, { line: "LRT Jakarta" });
    }

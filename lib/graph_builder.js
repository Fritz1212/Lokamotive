...
/**
 * Kelas GraphBuilder bertugas untuk membangun graph yang mengintegrasikan data dari 
 * berbagai layanan transportasi (Commuter Line, MRT, LRT, TransJakarta) serta menambahkan 
 * koneksi jalan kaki di antara node (stasiun/stop) yang berdekatan.
 *
 * @author Fritz Gradiyoga, Jessica Nathania Wenardi
 */
class GraphBuilder {
  /**
   * Membangun graph transportasi berdasarkan lokasi awal dan tujuan, dengan mengintegrasikan
   * data dari beberapa layanan transportasi.
   *
   * Proses ini meliputi:
   * - Pengambilan data jadwal Commuter Line dan pemrosesan node serta edge-nya.
   * - Integrasi data MRT, LRT, dan TransJakarta dengan menambahkan node dan edge.
   * - Penambahan koneksi jalan kaki (walking edges) secara dinamis antara node yang berdekatan.
   * - Penambahan node untuk lokasi pengguna (current location) dan tujuan (destination).
   *
   * @async
   * @param {string} start - Nama lokasi awal (Your Location).
   * @param {string} destination - Nama lokasi tujuan.
   * @param {number} startLat - Latitude lokasi awal.
   * @param {number} startLng - Longitude lokasi awal.
   * @param {number} destinationLat - Latitude lokasi tujuan.
   * @param {number} destinationLng - Longitude lokasi tujuan.
   * @return {Promise<Graph>} - Graph yang telah dibangun dengan node dan edge dari berbagai layanan.
   */
  async buildGraph(start, destination, startLat, startLng, destinationLat, destinationLng) {
   ...
   /**
   * Menambahkan edge jalan kaki antara node pada graph jika jarak antar node kurang dari 0.5 km.
   *
   * Menghitung jarak menggunakan metode haversine dan menambahkan edge dengan waktu perjalanan
   * berdasarkan kecepatan jalan kaki (diasumsikan 5 km/jam).
   *
   * @param {Graph} graph - Graph yang akan ditambahkan edge jalan kaki.
   */
...
/**
 * Fungsi untuk menghitung waktu tempuh (dalam menit) antara waktu keberangkatan dan kedatangan.
 *
 * Jika hasil perhitungan negatif, maka diasumsikan waktunya melampaui tengah malam.
 *
 * @param {string} departs - Waktu keberangkatan dalam format "HH:mm".
 * @param {string} arrives - Waktu kedatangan dalam format "HH:mm".
 * @return {number} - Waktu tempuh dalam menit.
 */
